...
class Order(Name, ID):
    ...
    def get_info(self):
        print(f"Order ID: {self.id}, Name: {self.name}, Cost: {self.cost}")
...
class System:
    ...
    def create_order(self, order_name, branch_id, cost):
        ...
        if available_employees:
            selected_employees = random.sample(available_employees, min(len(available_employees), 2))  # беремо до 2 працівників
    ...
    # Додаємо методи для роботи з адміном:
